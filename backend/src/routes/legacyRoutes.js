...
if (!JWT_SECRET || JWT_SECRET.length < 32) {
  logger.error({ event: 'config_error' }, 'JWT_SECRET es obligatorio y debe tener al menos 32 caracteres. Backend detenido.');
  process.exit(1);
}
...
const JLEON_PASSWORD_HASH = '$2a$10$I0slu/Cwht2gyvVXKsO3QOuaBa26u1sfeG/PTgpzFPAFvJmce0fhy';
...
const toPositiveInt = (value, fallback) => (Number.isFinite(value) && value > 0 ? value : fallback);
...
const isRetryableGoogleError = (error) => {
  ...
  const errCode = String(error?.errno || error?.code || '').toUpperCase();
  return ['ECONNRESET', 'ETIMEDOUT', 'EAI_AGAIN', 'ENOTFOUND', 'ECONNREFUSED'].includes(errCode);
};

const withGoogleRetry = async (label, fn) => {
  let attempt = 0;
  while (attempt <= GOOGLE_RETRY_COUNT) {
    attempt += 1;
    let timer;
    try {
      const timeoutPromise = new Promise((_, reject) => {
        timer = setTimeout(() => {
          const err = new Error(`${label} timeout (${GOOGLE_TIMEOUT}ms)`);
      ...
      if (attempt > GOOGLE_RETRY_COUNT || !isRetryableGoogleError(error)) {
        throw error;
      }
      await sleep(GOOGLE_RETRY_DELAY * attempt);
    } finally {
      if (timer) clearTimeout(timer);
    }
  }
  throw new Error(`${label} error no recuperable`);
};

const CACHE_KEY_STOCK = 'cache:stock';
const CACHE_KEY_STOCK_CATALOG = 'cache:stock:catalog';
const CACHE_KEY_OSO_ORDERS = 'cache:oso:orders';
const invalidateOperationalCaches = () => invalidateCacheByPrefix(['cache:stock', 'cache:oso']);

const getExternalErrorMessage = (error, fallback = 'Error consultando servicio externo') => {
  if (error?.code === 'GOOGLE_TIMEOUT') {
    return 'Tiempo de espera agotado consultando Google Sheets';
  }
  if (isRetryableGoogleError(error)) {
    return 'Google Sheets no responde temporalmente';
  }
  return fallback;
};
...
const getSheetData = async (sheets, spreadsheetId, tabName, valueRenderOption = 'UNFORMATTED_VALUE') => {
  const { data } = await withGoogleRetry('sheets.spreadsheets.values.get', () => sheets.spreadsheets.values.get({
    spreadsheetId,
    range: tabName,
    valueRenderOption
  }));
...
const writeProductoToSheet = async (origen, producto, action = 'upsert') => {
  ...
  if (action === 'delete') {
    ...
    await withGoogleRetry('sheets.spreadsheets.batchUpdate', () => sheets.spreadsheets.batchUpdate({
      spreadsheetId,
      requestBody: {
        requests: [
          {
  ...
  if (rowIndex > 1) {
    await withGoogleRetry('sheets.spreadsheets.values.update', () => sheets.spreadsheets.values.update({
      spreadsheetId,
    ...
    await withGoogleRetry('sheets.spreadsheets.values.append', () => sheets.spreadsheets.values.append({
      spreadsheetId,
      range: tabName,
      valueInputOption: 'USER_ENTERED',
...
const { requireAuth, requireAdmin, requireCotizadorStockAdmin, requireAdminOrIntcomexCompras, requireOwnerOrAdmin } = buildAuthMiddlewares({
  ...
  getSessionHeaders,
  validateSession,
  onMissingSession: async (user, req) => {
...
const isDbLoginRateLimited = async ({ usuario, req }) => {
  ...
  const windowMin = Number.isFinite(LOGIN_RATE_LIMIT_WINDOW_MIN) && LOGIN_RATE_LIMIT_WINDOW_MIN > 0
    ...
    : 15;
  const maxAttempts = Number.isFinite(LOGIN_RATE_LIMIT_MAX) && LOGIN_RATE_LIMIT_MAX > 0
    ? LOGIN_RATE_LIMIT_MAX
    : 5;
  const result = await pool.query(
    `SELECT
       COUNT(*) FILTER (WHERE usuario = $1) AS user_failures,
       COUNT(*) FILTER (WHERE ip_address = $2) AS ip_failures
     FROM login_logs
     WHERE success = false
       AND created_at >= NOW() - ($3 || ' minutes')::interval`,
    [usuario || null, ip || null, String(windowMin)]
  );
  const row = result.rows[0] || {};
  const userFailures = Number(row.user_failures || 0);
  const ipFailures = Number(row.ip_failures || 0);
  return userFailures >= maxAttempts || ipFailures >= maxAttempts;
};

const resolveCotizacionOwnerId = async (req) => {
  const result = await pool.query('SELECT usuario_id FROM cotizaciones WHERE id = $1', [req.params.id]);
...
// LOGIN
app.post('/api/login', loginRateLimiter, validateLoginInput, async (req, res) => {
  try {
    const { usuario, password } = req.body;
    const { sessionId, deviceId } = getSessionHeaders(req);
    const dbLimited = await isDbLoginRateLimited({ usuario, req });
    if (dbLimited) {
      return res.status(429).json({ error: 'Demasiados intentos de inicio de sesion. Intenta mas tarde.' });
    }
    ...
    logError(req, error, 'login_failed');
...
app.patch('/api/usuarios/me/password', authenticateToken, validatePasswordInput, async (req, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(403).json({ error: 'Permiso denegado' });
...
app.patch('/api/usuarios/:id/password', authenticateToken, requireAdmin, validatePasswordInput, async (req, res) => {
  try {
    const { id } = req.params;
    const { password } = req.body;
...
app.get('/api/oso/orders', authenticateToken, requireAdminOrIntcomexCompras, async (req, res) => {
  try {
    ...
    const payload = { count: orders.length, orders };
    cacheSet(CACHE_KEY_OSO_ORDERS, payload, OSO_ORDERS_CACHE_TTL_MS);
...
app.get('/api/productos', authenticateToken, async (req, res) => {
  try {
    ...
    if (origen) {
      params.push(origen);
      whereParts.push(`origen = $${params.length}`);
    } else {
      whereParts.push('activo = true');
    }
    const whereSql = whereParts.length ? `WHERE ${whereParts.join(' AND ')}` : '';

    if (isAdmin && hasPagination) {
    ...
    logError(req, error, 'productos_list_failed');
    res.status(500).json({ error: 'Error del servidor' });
  }
});
...
app.post('/api/productos', authenticateToken, requireCotizadorStockAdmin, validateProductoInput, async (req, res) => {
  ...
  try {
    ...
    logError(req, error, 'producto_create_failed');
    res.status(500).json({ error: 'Error del servidor' });
  } finally {
    client.release();
  }
...
app.post('/api/cotizaciones', authenticateToken, validateCotizacionInput, async (req, res) => {
  ...
  try {
    ...
    await client.query('BEGIN');
    if (!isAdmin) {
      const userResult = await client.query('SELECT gp, gp_qnap, gp_axis, partner_category FROM usuarios WHERE id = $1', [usuarioId]);
      const userRow = userResult.rows[0] || {};
      const gpQnap = parseGpValue(userRow.gp_qnap ?? userRow.gp ?? 0.15, 0.15);
      const gpAxis = parseGpValue(userRow.gp_axis ?? userRow.gp ?? 0.15, 0.15);
      const partnerCategory = userRow.partner_category || 'Partner Autorizado';
      ...
      const requestedProductIds = [...new Set(
        requestedItems
          .map(item => parseInt(item?.producto_id, 10))
          .filter(id => Number.isFinite(id))
      )];
      const productsResult = requestedProductIds.length > 0
      ...
      for (const item of requestedItems) {
        ...
        if (!Number.isFinite(productoId)) {
          const badRequest = new Error('producto_id invalido');
          badRequest.status = 400;
          throw badRequest;
        }
        const cantidad = parseInt(item.cantidad || item.cant || 1, 10);
        const producto = productById.get(productoId);
        if (!producto) {
          const badRequest = new Error(`Producto no encontrado: ${productoId}`);
          badRequest.status = 400;
          throw badRequest;
        }
        const origenValue = producto.origen || DEFAULT_ORIGIN;
    ...
    logError(req, error, 'cotizacion_create_failed');
    if (error?.status === 400) {
      return res.status(400).json({ error: error.message || 'Solicitud invalida' });
    }
    res.status(500).json({ error: 'Error del servidor' });
  } finally {
    client.release();
...
app.patch('/api/cotizaciones/:id/estado', authenticateToken, requireOwnerOrAdmin(resolveCotizacionOwnerId, 'Cotizacion no encontrada'), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    logError(req, error, 'cotizacion_estado_failed');
    res.status(500).json({ error: 'Error del servidor' });
  }
});
...
app.get('/api/cotizaciones', authenticateToken, async (req, res) => {
  try {
    ...
    const whereSql = isAdmin ? '' : 'WHERE c.usuario_id = $1';
    ...
    if (!hasPagination) {
      const result = await pool.query(
        `SELECT c.*, u.role AS usuario_role
         FROM cotizaciones c
      ...
      const payload = await attachItems(result.rows);
      return res.json(payload);
    }
    ...
    const data = await attachItems(dataResult.rows);
    res.json({
      data,
      meta: {
        page,
    ...
    logError(req, error, 'cotizacion_list_failed');
    res.status(500).json({ error: 'Error del servidor' });
  }
});

// COTIZACIONES - Eliminar
app.delete('/api/cotizaciones/:id', authenticateToken, requireOwnerOrAdmin(resolveCotizacionOwnerId, 'Cotizacion no encontrada'), async (req, res) => {
  const client = await pool.connect();
  try {
    const { id } = req.params;
    await client.query('BEGIN');
    ...
    logError(req, error, 'cotizacion_delete_failed');
    res.status(500).json({ error: 'Error del servidor' });
  } finally {
    client.release();
  }
